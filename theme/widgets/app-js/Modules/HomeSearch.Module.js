        ...
        onKeyup:()=>{
            ...
            if (searchKeyword==='') {
                ...
                if (results.length===0) {
                    ...
                    resultsUI = resultsUI+'<ul class="mg-top-2">';
                    for (var i = 0; i < results.length; i++) {
                        let resultItem = results[i];
                        let realPath = '/'+PathSvc.toQueryParams(resultItem.path);
                        resultsUI = resultsUI+'<li class="border-bottom-1 border-thin border-light hover-bg-light"><a class="no-decor text-color-dark" href="'+realPath+'"><div class="pd-top-5 pd-bottom-5 subtext text-6 fw-4">'+resultItem.name+'</div></a></li>';
                    }
                    resultsUI = resultsUI+'</ul>';
                }
